<template>
  ...
  <div>
    <div class="lott-right-top1">
      <button>今日开奖</button>
      <p>
        <router-link tag="a" target="_blank" :to='"/trendChart/1804/"+$route.params.group'>走势图</router-link>
        <i>|</i>
        <!-- <span>玩法说明</span> -->
      </p>
    </div>
      ...
      <ul>
        <li v-for="(item,index) in getPastOpens" :key="index">
          ...
          <span class="lott-right-top2-span2 ssc">
            <i>{{item.n1 &lt; 10 ? "0"+item.n1 : item.n1}}</i>
            <i>{{item.n2 &lt; 10 ? "0"+item.n2 : item.n2}}</i>
            <i>{{item.n3 &lt; 10 ? "0"+item.n3 : item.n3}}</i>
            <i>{{item.n4 &lt; 10 ? "0"+item.n4 : item.n4}}</i>
            <i>{{item.n5 &lt; 10 ? "0"+item.n5 : item.n5}}</i>
          </span>
